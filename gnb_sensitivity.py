...
cmap_S = plt.get_cmap('autumn')
cmap_D = plt.get_cmap('Reds')

plt.bar(index, score_changes_S, bar_width, label='Sport', color=[cmap_S((i+15)/(len(index)+20)) for i, _ in enumerate(index)])
plt.bar(index + bar_width, score_changes_D, bar_width, label='Discipline', color=[cmap_D((len(index)+15-i)/(len(index)+25)) for i, _ in enumerate(index)])
...
plt.title('Sensitivity Analysis of Factors in Gaussian Naive Bayes Classifier', fontsize=30)
